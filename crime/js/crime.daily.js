...
var json_where = "$select=date, community_area, primary_type, location_description, arrest, domestic&$where=year>2014&$limit=500000&$$app_token=BWWPzJPWah0RdMqRqbWEw53eb"; //$where=date>'2015-08-01T12:00:00'
...
$('#area_select').on('change',function(e){
	...
	if(sValue == 0){
		...
		$("title").html(sText + " Crimes per Day");
...
var tip = d3.tip()
	.attr('class', 'd3-tip')
	.offset([-10, 0])
	.html(function (d) {return "<span style='color: white'>" + formatDate(d.x) + "</span>: "  + d.y; });
...
function drawChart(url){
	...
	d3.json(url, function(error, data){ 
		...
		var startDate = d3.time.day.offset(new Date(), -148);
		var endDate = d3.time.day.offset(new Date(), -7);
	    ...
	    lineChart
			.width(816)
			.height(580)
			.margins({top: 20, right: 40, bottom: 70, left: 50})
			.dimension(dateValue)
		...
		lineChart.xAxis().ticks(10).tickSize(7,0).tickFormat(d3.time.format("%m/%d/%y"));	
		lineChart.yAxisLabel('TOTAL PER DAY');	
		lineChart.xAxisLabel('CALENDAR DAYS');
		...
		function rateOfChange(slope, intercept, xLength) { 
			...
			if(slope>0){
				celeration = "X" + roundCel((intercept+slope*(xLength/2))/(intercept+slope*(xLength/2-7)));				
			}else if(slope<0){
				celeration = "/" + roundCel((intercept+slope*(xLength/2-7))/(intercept+slope*(xLength/2)));
			}else{
				celeration = "X1";
			}
